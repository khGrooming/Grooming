		...
		<div class="container" style="margin-top:50px;">
			...
			<div style="width: 100%;">
				<div style="width: 30%; float: left;">
					<span>모집 기간 : </span><span>${grooming.groomingSd }
									~ ${grooming.groomingEd }</span>
				</div>
				<div style="float: right;">
					<img src="${contextPath }/resources/views/images/repotBtn.png" height="22px" data-toggle='modal' data-target='#declareForm' style="margin-bottom: 3px" id="apply">
				</div>
			...
			<div style="width: 100%; position: relative;">
				<div style="height: 300px;text-align: center;background: #c0d2c0;">
					<img src="${contextPath }/resources/upGroomingFiles/${grooming.groomingImg}">
				</div>
				<div class="status" style="position: absolute; top: 0; right: 0; background: white; margin: 10px 15px">
			...
			<div>
				<div style="width: 98%; margin-left:10px;">
					<span style="font-size:35px;">${grooming.groomingTitle }</span>
					...
					<span style="float: right;">
					 조회수 : ${grooming.count}회
					</span>
					<br>
					<c:forEach var="t" items="${tag }">
						<span style="padding: 3px 9px 3px 8px; background: gray; color:white;">${t.tagName }</span>
					</c:forEach>
					<br>
					<br>
					<h5>${grooming.groomingIntroduce }</h5>
					
					<p style="margin-top: 70px; margin-bottom: 0;">- 상세내용</p>
				...
				<div style="margin-top: 70px;    margin-left: 10px;">
					<div  style="width: 40%;">
						<div >
							<span style="margin-right: 5px;">- 그룹 타입:</span>
							<span>${grooming.groomingType }</span>
						</div>
						<div style="margin-top: 30px;">
							<span style="margin-right: 5px;">- 스터디 기간:</span>
							<span>${grooming.studySd } ~ ${grooming.studyEd }</span>
						</div>
						<div style="margin-top: 30px;">
							<span style="margin-right: 5px;">- 예치금:</span>
							<span>${grooming.money } 원</span>
						</div>
						<div style="margin-top: 30px;">
							<span style="margin-right: 5px;">- 모집 인원:</span>
							<span>${grooming.currentP }/${grooming.groomingP } 명</span>
						</div>
					</div>
				
					<div style="width: 40%;margin-top: 30px;">
						<p style="margin-bottom: 0;">-호스트 정보</p>
						<div style=" width: 100%">
							<div style="width: 33%; float: left">
								<div style="width: 100px; height: 100px; border-radius: 50%; border:1px solid gray; margin:20px 20px;">
									<img src="${contextPath }/resources/upprofileFiles/${member.memberPhoto}" width="100px" style="border-radius: 50%;">
								
								</div>
								<p style="text-align: center;">${member.memberNickName }</p>
							</div>
							<div style="float: left; width:66%; margin-top:20px">
								<div>
									<c:forEach var="s" items="${spec }">
										<c:if test="${s.specCNo eq 'SC00001' }">
											<span class="specStyle specS">${s.specName }</span>
										</c:if>
									</c:forEach>
								</div>
								<div style="margin-top:12px;">
									<c:forEach var="s" items="${spec }">
										<c:if test="${s.specCNo eq 'SC00002' }">
											<span class="specStyle specC">${s.specName }</span>
										</c:if>
									</c:forEach>
								</div>
								<div  style="margin-top:12px;">
									<c:forEach var="s" items="${spec }">
										<c:if test="${s.specCNo eq 'SC00003' }">
											<span class="specStyle specCE">${s.specName }</span>
										</c:if>
									</c:forEach>
								</div>
			...
			<div style="position: relative;">
				...
				<div style="text-align: center;">			
					...
					<c:if test="${grooming.status eq 'Y' }">
						<c:if test="${loginUser.memberNo ne grooming.memberNo  }">
							<c:if test="${(!empty memberNoList) || (grooming.groomingEd lt grooming.groomingNd)  || (grooming.groomingP eq grooming.currentP) || (grooming.status eq 'B') }">
								<c:out value="<button data-toggle='modal' data-target='#applyForm' id='apply1' disabled>신청하기</button>" escapeXml="false" />
							</c:if>
							...
							<c:if test="${(empty memberNoList) && (grooming.groomingEd gt grooming.groomingNd) && (grooming.groomingP gt grooming.currentP) && (grooming.status eq 'Y') }">
								<c:out value="<button data-toggle='modal' data-target='#applyForm' id='apply1'>신청하기</button>" escapeXml="false" />
							</c:if>
						</c:if>
					</c:if>
					<c:if test="${loginUser.memberNo eq grooming.memberNo }">
						...
						<!-- 글의 상태 버튼 -->
							...
							<button class="adBtn" onclick="location.href='${gupdate}'">수정</button>
							<button class="adBtn"id="deleteGrooming" type="button">삭제</button>
							<button class="adBtn"id="limit" type="button">마감</button>
				...
				<div style="width: 8%; margin: 0 auto; position: absolute; top:0; cursor: pointer;">
					<p onclick="location.href='${goMain}'" >목록으로</p>
				</div>
				
				<div style="position: absolute; right: 0; cursor: pointer; top:0;">
					<c:if test="${loginUser.memberNo eq grooming.memberNo }">
						<!-- 신청자 리스트  확인 버튼 -->
						<p data-toggle="modal" id="applicantList">신청자 리스트</p>
